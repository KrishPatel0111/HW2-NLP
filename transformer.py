...
class Transformer(nn.Module):
    ...
    def forward(self, indices):
        ...
        """
        # Forward pass and returns log probabilities and attention maps
...
class TransformerLayer(nn.Module):
    def __init__(self, d_model, d_internal):
        ...
        """
        super().__init__()
        #linear layers to generate Q, K, V matrices
        ...
        self.ffn = nn.Sequential(
            ...
            nn.GELU(),
            nn.Linear(5 * d_model, d_model, device="cuda" if torch.cuda.is_available() else "cpu"),
        )
...
def train_classifier(args, train, dev):
    ...
    #hyperparameters and model, optimizer, loss function initialization
    d_model = 128
    d_internal = 32
    num_layers = 1
    num_classes = 3
    ...
    for epoch in range(num_epochs):
        ...
        print(f"Epoch {epoch}: loss={loss_this_epoch/len(train):.4f}")
    model.eval()
    return model
